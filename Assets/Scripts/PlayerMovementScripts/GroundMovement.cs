...
public class GroundMovement : MonoBehaviour {
    ...
    private void airMovement() {
        ...
        Vector3 accelerateResult = Accelerate(desiredMoveDir, rb.velocity, airAcceleration, maxAirVelocity);

        rb.AddForce(accelerateResult);
    }
    ...
    private Vector3 Accelerate(Vector3 accelDir, Vector3 prevVelocity, float accelerate, float max_velocity) {
        ...
        float accelVel = (accelerate/rb.mass) * Time.fixedDeltaTime; // Accelerated velocity in direction of movment

        // If necessary, truncate the accelerated velocity so the vector projection does not exceed max_velocity
        if (projVel + accelVel > max_velocity)
            accelVel = max_velocity - projVel;

        //return acceleration vector, which will be used to apply rigid body force.
    ...
    private void mouseLook() {
        ...
        //Rotate the camera up and down, which is a child of this object!
        Camera childCam = this.transform.GetComponentInChildren<Camera>(false);
        if (childCam != null) {
            childCam.transform.localEulerAngles = new Vector3(angleX, 0, 0);
        }
    }
